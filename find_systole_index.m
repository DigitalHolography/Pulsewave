function[sys_index_list, fullPulseWave] =
    find_systole_index(video, path, maskArtery)
...
[ ~, sys_index_list ] =
    findpeaks(diff_signal, 1
              : length(diff_signal), 'MinPeakHeight',
                max(diff_signal) * PW_params.systoleThreshold);

% figure(3) % plot(diff_signal);
...
% tmp = ifft(tmp);
% figure(42) % plot(w, real(tmp), w, diff_signal);
% pseudo_period = length(w) / omega1;
% distance_between_peaks = 0.7 * pseudo_period;
% % [ ~, index_list ] =
    findpeaks(diff_signal, 1
              : length(diff_signal), 'MinPeakDistance', distance_between_peaks,
                'SortStr', 'descend');
% [ ~, index_list_tmp ] =
    findpeaks(tmp, 1
              : length(tmp), 'MinPeakDistance', distance_between_peaks,
                'SortStr', 'descend');
...
% end % sys_index_list = sort(sys_index_list, 'ascend');
