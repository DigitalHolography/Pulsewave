function [spec_plot, delt_spec_plot] = Show_spectrum(maskArtery,maskBackground,maskSection,SH,ToolBox)
    ...
    arterySpectrum = squeeze(mean(SH .*maskArtery .*maskSection, [1, 2]));
    backgroundSpectrum = squeeze(mean(SH .*maskBackground .*maskSection, [1, 2]));
    deltaSpectrum = arterySpectrum - backgroundSpectrum;

    spec_plot = figure(57);

    x = fullfreq;
    y = double(10 * log(fftshift(spectrum .*maskSection / sum(spectrum(fftshift(~exclude))))));
    ...
    lorentzEqn = @(a, x) ...
        10 * log(1 ./ ((1 + abs(x / a) .^ 2)) / sum(1 ./ ((1 + abs(fullfreq(~exclude) / a) .^ 2))));
    l = fittype(lorentzEqn);
    fl = fit(x', double(yd), l, Exclude = exclude); % fit a lorentzian model to the delta spectrum.
    ...
    xline(f2, 'k--', 'LineWidth', 2)
    hold off
    legend('artery spectrum', 'background spectrum', 'delta spectrum', ['lorentz model 1/(1+(x/a)^2)', 'a = ', num2str(fl.a)]);
    title('Spectrum');
    fontsize(gca, 14, "points");
    xlabel("Frequency (kHz)", 'FontSize', 14);
    ylabel("S(f) (dB)", 'FontSize', 14);
