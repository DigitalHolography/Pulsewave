...
flag = 0;
% Check if image is normalized between 0 and
    1 if min (image, [], 'all')<0 || max(image, [], 'all')> 1 Im_max =
    max(image, [], 'all');
...
flag = 1;
    ...
    if (borderAmount == 0) a = 1;
b = size(image, 1);
c = 1;
d = size(image, 2);
else a = ceil(size(image, 1) * borderAmount);
...
if flag
  == 1 corrected_image = Im_min + (Im_max - Im_min).*corrected_image;
end
