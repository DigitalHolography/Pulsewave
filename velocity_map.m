function [] = velocity_map(maskArtery, maskVein, v_RMS_one_cycle, onePulseVideoM0, ToolBox)

PW_params = Parameters_json(path);
veins_analysis = PW_params.veins_analysis;

% TRUE MIN and MAX V_RMS but not realistic
Im = rescale(mean(onePulseVideoM0, 3));
...
v_artery = sum(v_RMS_one_cycle .* maskArtery, [1 2]) / nnz(maskArtery);
v_vein = sum(v_RMS_one_cycle .* maskVein, [1 2]) / nnz(maskVein);
...
ImgM0 = rescale(mean(onePulseVideoM0, 3));
...
flowVideoRGB_one_cycle = zeros(size(v_RMS_one_cycle, 1), size(v_RMS_one_cycle, 2), 3, size(v_RMS_one_cycle, 3));
videoM0_one_cycle_norm = rescale(onePulseVideoM0);

if veins_analysis

    for ii = 1:size(v_RMS_one_cycle, 3)
        v = v_RMS_one_cycle(:, :, ii);
...
v_histo_artery = round(v_RMS_one_cycle .* maskArtery);
v_min = min(v_histo_artery, [], 'all');
v_max = max(v_histo_artery, [], 'all');

X = linspace(v_min, v_max, v_max - v_min + 1);
n = size(X, 2);
histo = zeros(size(X, 2), size(v_RMS_one_cycle, 3));

for t = 1:size(v_RMS_one_cycle, 3)

    for x = 1:size(v_RMS_one_cycle, 1)

        for y = 1:size(v_RMS_one_cycle, 2)
...
v_histo_veins = round(v_RMS_one_cycle .* maskVein);
