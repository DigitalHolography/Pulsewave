function [] = bloodVolumeRate(maskArtery, maskVein, ~, ~, v_RMS, dataM0, videoM0_from_holowaves, ToolBox, k, path, flagBloodVelocityProfile)
    ...
    deltr = PW_params.radius_gap * (M + N) /nbCircles;
    for i = 1:nbCircles
        rad1 = (PW_params.radius_ratio - PW_params.radius_gap) * (M + N) / 2 + (i-1) * deltr ;
        rad2 = rad1 + deltr ;
        ...
        % save mask image
        create_mask_section(mean_M0 , maskArtery,rad1,rad2,sprintf('mask_artery_section_circle_%d.png',i), ToolBox, path);
    end
    close(156);
