classdef OneCycleClass properties reference(1,
                                            :) cell
    %
    M0 AVI reference_norm(1,
                          :) cell
    %
    M0 norm AVI reference_interp(1,
                                 :) cell
    %
    M0 AVI reference_norm_interp(1,
                                 :) cell
    %
    M0 norm AVI dataM2M0(1,
                         :) cell
    % RMS M2 /
    M0 dataM1M0(1,
                :) cell
    % AVG M1 /
    M0 dataM2M0_interp(1,
                       :) cell
    % RMS M2 /
    M0 dataM1M0_interp(1,
                       :) cell
    % AVG M1 /
    M0 dataM0(1,
              :) cell
    %
    M0 raw dataM0_interp(1,
                         :) cell
    %
    M0 raw dataM1(1,
                  :) cell
    ...
    M2 raw dataSH(1,
                  :) cell
    %
    SH raw dataSH_interp(1,
                         :) cell
    %
    SH raw directory
    char nbFiles{mustBeNumeric, mustBePositive} filenames(1,
                                                          :) cell k double %
    interpolaton parameter load_logs char %
    For sectioning video_loaded pictureSection videoSection flag_FlatField
    flag_SH_analysis flag_PulseWave_analysis isdone_flatfield
    % %
    FIXME : relancer a chaque rendering ToolBoxmaster ToolBoxClass

            end

            methods function obj = OneCycleClass(path)
